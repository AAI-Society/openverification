...
substitution, and that no agent found a way to retroactively redact or edit one.
...
written inside the perimeter it describes. Seven per cent of evaluated
transcripts were spoofed. A record committed as it is written, to monitors
...
| `audit-tampering` | Context compaction abused, code injected into the evaluator's container, adversarial output printed; roughly seven per cent of evaluated transcripts spoofed |
